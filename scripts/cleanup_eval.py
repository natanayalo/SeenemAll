...
def deduplicate_and_rebalance():
    ...
    data.sort(
        key=lambda x: (
            not x.get("case_id", "").startswith("v2_case"),
            x.get("case_id", ""),
        )
    )
    ...
    for entry in data:
        ...
        if golden_key not in seen_golden_sets:
            ...
            print(
                f"Skipping duplicate: {entry['case_id']} (Duplicate of {seen_golden_sets[golden_key]})"
            )
    ...
    for entry in unique_data:
        ...
        if "Crime" not in (entry.get("genre_override", "") or "") and bucket not in [
            "semantic_query_tv",
            "constrained_multi_filter",
        ]:
            if any(item["id"] == 96677 for item in entry["golden_set"]):
                entry["golden_set"] = [
                    item for item in entry["golden_set"] if item["id"] != 96677
                ]
        ...
        if any(item["id"] == 71738 for item in entry["golden_set"]):
            ...
            if count_71738 > 8 and bucket == "semantic_query_tv":
                entry["golden_set"] = [
                    item for item in entry["golden_set"] if item["id"] != 71738
                ]
    ...
    for entry in unique_data:
        if len(entry["golden_set"]) < 4:
            ...
            if "psychological thrillers" in (entry.get("query") or "").lower():
                ...
                entry["golden_set"].append(
                    {"id": 204541, "title": "Silo", "media_type": "tv"}
                )
            elif "superhero" in (entry.get("query") or "").lower():
                entry["golden_set"].append(
                    {"id": 95557, "title": "Invincible", "media_type": "tv"}
                )
            elif "family adventure" in (entry.get("query") or "").lower():
                entry["golden_set"].append(
                    {"id": 11, "title": "Star Wars", "media_type": "movie"}
                )
