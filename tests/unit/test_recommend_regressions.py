...
from api.routes.recommend import _normalize_merged_intent, _apply_explicit_query_overrides
...
def test_light_drama_preserves_mood_and_no_wrong_genres():
    ...
    intent = IntentFilters(raw_query="light drama movies", genres=["Drama"], moods=["light"], media_types=["movie"])
...
def test_light_family_injects_genres():
    intent = IntentFilters(raw_query="light family adventure", genres=["Adventure"], moods=["light"], media_types=["movie"])
...
def test_animation_provenance_trusts_llm():
    ...
    intent = IntentFilters(raw_query="animated comedy", genres=["Animation", "Comedy"], media_types=["movie"])
...
def test_animation_stripped_if_not_in_llm_or_query():
    ...
    intent = IntentFilters(raw_query="spooky thriller", genres=["Animation", "Horror"], media_types=["movie"])
...
def test_maturity_explicit_llm_is_respected():
    intent = IntentFilters(raw_query="safe movies for 10 year olds", maturity_rating_max="PG", media_types=["movie"])
...
def test_maturity_reset_if_not_explicit():
    intent = IntentFilters(raw_query="action movies", maturity_rating_max="G", media_types=["movie"])
