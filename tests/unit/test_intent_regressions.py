...
def test_normalize_merged_intent_strips_legacy_animation():
    ...
    normalized = _normalize_merged_intent(
        intent, "feel-good movie", llm_intent=llm_intent
    )
...
def test_apply_explicit_query_overrides_respects_llm_maturity():
    ...
    intent = IntentFilters(
        raw_query="space adventure", media_types=["movie"], maturity_rating_max="PG"
    )
    llm_intent = Intent(maturity_rating_max="PG")

    overridden = _apply_explicit_query_overrides(
        intent, "space adventure", llm_intent=llm_intent
    )
...
def test_apply_explicit_query_overrides_strips_unintended_restrictive_cap():
    ...
    overridden = _apply_explicit_query_overrides(
        intent, "psychological thriller", llm_intent=llm_intent
    )
...
def test_apply_explicit_query_overrides_family_signal():
    ...
    intent = IntentFilters(
        raw_query="family movies", media_types=["movie"], maturity_rating_max=None
    )
